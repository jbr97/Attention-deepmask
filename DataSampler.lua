...
-- class members
  ...
  local step = self.gSz/self.fSz
  self.conv = nn.SpatialConvolution(1,1,step,step,step,step)
  self.conv.bias:zero()
  self.conv.weight:fill(1/step/step)
...
function DataSampler:attentionSampling()
  ...
  local x1, x2, y1, y2 = box[1]+1, box[1] + box[3], box[2]+1, box[2] + box[4]
  if (x1<=0) then x1=1 end
  if (x2>gSz) then x2=gSz end
  if (y1<=0) then y1=1 end
  if (y2>gSz) then y2=gSz end
  att[{{y1,y2},{x1,x2}}] = 1
--[[  
  print(#mask)
print(x1)
...
--[[
  local retatt = torch.FloatTensor(1,self.fSz,self.fSz):zero()
  for i = 1,self.fSz do
    for j = 1,self.fSz do
      for k = 1,step do
        for l = 1,step do
          retatt[1][i][j]=retatt[1][i][j]+att[i*step-step+k][j*step-step+l]
        end
      end
      retatt[1][i][j]=retatt[1][i][j]/step/step
    end
  end
...
att=att:view(1,self.gSz,self.gSz)
--print(#att)
local conv=self.conv:clone()
--print(conv)
--print(#conv.weight)
  local retatt = conv:forward(att)
