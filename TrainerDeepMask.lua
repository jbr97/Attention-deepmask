...
--------------------------------------------------------------------------------
-- function: init
function Trainer:__init(model, criterion, config, BCE)
  -- training params
  self.config = config
  self.model = model
  ...
  -- meters
  self.lossmeter1  = LossMeter()
  self.lossmeter2  = LossMeter()
  self.lossmeter3  = LossMeter()
...
function Trainer:train(epoch, dataloader)
  ...
  self.lossmeter1:reset()
  self.lossmeter2:reset()
  self.lossmeter3:reset()
  ...
  local fevalatt   = function() return self.BCE.output,   self.ga end
  ...
  for n, sample in dataloader:run() do
    ...
    local lossbatch = CRI:forward(outputs, self.labels)

    model:zeroGradParameters()
    local gradOutputs = CRI:backward(outputs, self.labels)
    ...
    -- update loss
    if sample.head == 1 then self.lossmeter1:add(lossbatch) end
    if sample.head == 2 then self.lossmeter2:add(lossbatch) end
    if sample.head == 3 then self.lossmeter3:add(lossbatch) end
    
  end

  -- write log
  local logepoch =
    string.format('[train] | epoch %05d | s/batch %04.2f | maskloss: %07.5f | scoreloss: %07.5f | attloss: %07.5f',
      epoch, timer:time().real/dataloader:size(),self.lossmeter1:value(),self.lossmeter2:value(),self.lossmeter3:value())
  print(logepoch)
